...
describe('test Purchases', () => {
  ...
  describe('test listPurchases', () => {
    test('test api call', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get(
          '/purchases?iccid=7670512497&afterDate=blanditiis&beforeDate=dignissimos&afterCursor=corporis&limit=9&after=5&before=3',
        )
        .reply(200, { data: {} });
      return sdk.purchases
        .listPurchases({
          iccid: '7670512497',
          afterDate: 'blanditiis',
          beforeDate: 'dignissimos',
          afterCursor: 'corporis',
          limit: 9,
          after: 5,
          before: 3,
        })
  ...
  describe('test getPurchaseConsumption', () => {
    test('test api call', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get('/purchases/beatae/consumption')
        .reply(200, { data: {} });
      return sdk.purchases
        .getPurchaseConsumption('beatae')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get('/purchases/at/consumption')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get('/purchases/error/consumption')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.purchases.getPurchaseConsumption('error'),
      ).rejects.toThrow();
    });
  });
});
