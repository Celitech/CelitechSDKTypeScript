...
describe('test Purchases', () => {
  ...
  describe('test listPurchases', () => {
    test('test api call', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get(
          '/purchases?iccid=2012343751&afterDate=occaecati&beforeDate=aut&afterCursor=laudantium&limit=5&after=6&before=2',
        )
        .reply(200, { data: {} });
  ...
  describe('test getPurchaseConsumption', () => {
    test('test api call', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get('/purchases/vel/consumption')
        .reply(200, { data: {} });
      return sdk.purchases
        .getPurchaseConsumption('vel')
        .then((r: any) => expect(r.data).toEqual({}));
    });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.celitech.net/v1')
        ...
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.purchases.getPurchaseConsumption('praesentium'),
      ).rejects.toThrow();
    });
  });
});
