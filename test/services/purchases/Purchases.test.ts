  ...
  describe('test listPurchases', () => {
    test('test api call', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get(
          '/purchases?iccid=2569835188&afterDate=rerum&beforeDate=provident&afterCursor=facilis&limit=3&after=7&before=2',
        )
        .reply(200, { data: {} });
  ...
  describe('test getPurchaseConsumption', () => {
    test('test api call', () => {
      const scope = nock('https://api.celitech.net/v1')
        ...
        .reply(200, { data: {} });
      return sdk.purchases
        .getPurchaseConsumption('nulla')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get('/purchases/quas/consumption')
        .reply(200, { data: {} });
