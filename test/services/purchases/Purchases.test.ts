...
describe('test Purchases', () => {
  ...
  describe('test listPurchases', () => {
    test('test api call', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get(
          '/purchases?iccid=6840238214&afterDate=voluptatem&beforeDate=nulla&referenceId=voluptatum&afterCursor=ratione&limit=9&after=7&before=8',
        )
        .reply(200, { data: {} });
      return sdk.purchases
        .listPurchases({
          iccid: '6840238214',
          afterDate: 'voluptatem',
          beforeDate: 'nulla',
  ...
  describe('test getPurchaseConsumption', () => {
    ...
    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get('/purchases/quae/consumption')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.celitech.net/v1')
        .get('/purchases/esse/consumption')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.purchases.getPurchaseConsumption('esse'),
      ).rejects.toThrow();
    });
  });
});
